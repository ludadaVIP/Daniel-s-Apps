"""French Vocab blueprint — French vocabulary by CEFR level.

Mounted at ``/api/french-vocab``. Data lives in ``backend/data/FrenchVocab/``.

Mirrors ``apps/esp_vocab/routes.py`` and ``apps/eng_vocab/routes.py`` exactly —
only the target language (French, ``fr``) and the translation language
(English, ``en``) differ. Same data layout, same reusable helpers from
``shared/vocab.py``, same Edge TTS cache layout.

TTS languages: ``fr`` (lemma + example) and ``en`` (translation).
"""
...
DEFAULT_DATA_DIR = Path(__file__).resolve().parents[2] / "data" / "FrenchVocab"
DATA_DIR = Path(os.environ.get("FRENCH_VOCAB_DATA_DIR", DEFAULT_DATA_DIR))
...
bp = Blueprint("french_vocab", __name__)


class FrenchVocabError(ValueError):
    def __init__(self, message: str, status_code: int = 400) -> None:
        super().__init__(message)
        self.status_code = status_code


@bp.errorhandler(FrenchVocabError)
def handle_error(error: FrenchVocabError):
    return jsonify({"error": str(error)}), error.status_code


def normalise_tts_language(value: Any) -> str:
    language = str(value or "fr").strip().lower()
    if language in {"french", "francais", "français", "fr-fr"}:
        return "fr"
    if language in {"english", "en-us", "en-gb"}:
        return "en"
    if language not in LANGUAGE_CONFIG:
        raise FrenchVocabError("Language must be 'fr' or 'en'.")
    return language


def tts_cache_key(text: str, language: str, voice: str) -> str:
    payload = f"french-vocab-edge-tts-v1\n{language}\n{voice}\n{text}"
...
def levels():
    if request.method == "OPTIONS":
        return "", 204
    return jsonify(load_levels(DATA_DIR, FrenchVocabError))
...
def level_detail(level_id: str):
    if request.method == "OPTIONS":
        return "", 204
    return jsonify(load_level(DATA_DIR, level_id, FrenchVocabError))
...
def group_detail(level_id: str, group_id: str):
    if request.method == "OPTIONS":
        return "", 204
    group = load_group(DATA_DIR, level_id, group_id, FrenchVocabError)
    return jsonify({
...
def create_tts_audio():
    ...
    if not text:
        raise FrenchVocabError("Text is required.")
    if len(text) > AUDIO_TEXT_LIMIT:
        raise FrenchVocabError(f"Text is too long. Keep it under {AUDIO_TEXT_LIMIT} characters.")
    ...
    audio_path = f"/audio/french-vocab/{output_path.relative_to(AUDIO_DIR).as_posix()}"
    ...
    try:
        ...
        raise FrenchVocabError(f"Could not generate audio with {voice}: {exc}", 502) from exc
