"""French 900 blueprint.

Mounted at ``/api/french-900`` in the unified backend.
Data lives in ``backend/data/French900/``.

900 French speaking sentences with Spanish parallel translations,
in nine progressive groups of 100. Two TTS languages: fr, es.
"""
...
DEFAULT_DATA_DIR = Path(__file__).resolve().parents[2] / "data" / "French900"
DATA_DIR = Path(os.environ.get("FRENCH_900_DATA_DIR", DEFAULT_DATA_DIR))
...
bp = Blueprint("french_900", __name__)


class French900Error(ValueError):
    def __init__(self, message: str, status_code: int = 400) -> None:
        super().__init__(message)
        self.status_code = status_code


@bp.errorhandler(French900Error)
def handle_error(error: French900Error):
...
def normalise_tts_language(value: Any) -> str:
    ...
    if language not in LANGUAGE_CONFIG:
        raise French900Error("Language must be 'fr' or 'es'.")
    return language


def tts_cache_key(text: str, language: str, voice: str) -> str:
    payload = f"french-900-edge-tts-v1\n{language}\n{voice}\n{text}"
...
def groups():
    if request.method == "OPTIONS":
        return "", 204
    course = load_900_index(DATA_DIR, DATA_FILE, French900Error)
    return jsonify({
        "id": course.get("id"),
...
def group_detail(group_id: str):
    if request.method == "OPTIONS":
        return "", 204
    return jsonify(load_900_group(DATA_DIR, DATA_FILE, group_id, French900Error))
...
def delete_group(group_id: str):
    if request.method == "OPTIONS":
        return "", 204
    course = load_course()
    group = delete_900_group(DATA_DIR, course, group_id, French900Error)
...
def create_tts_audio():
    ...
    if not text:
        raise French900Error("Text is required.")
    if len(text) > AUDIO_TEXT_LIMIT:
        raise French900Error(f"Text is too long. Keep it under {AUDIO_TEXT_LIMIT} characters.")
    ...
    audio_path = f"/audio/french-900/{output_path.relative_to(AUDIO_DIR).as_posix()}"
    ...
    try:
        ...
        raise French900Error(f"Could not generate audio with {voice}: {exc}", 502) from exc
