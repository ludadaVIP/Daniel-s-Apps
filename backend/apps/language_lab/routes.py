"""Language Output Lab blueprint.

Mounted at ``/api/lab`` in the unified backend. Configurable via
``LANGUAGE_LAB_DATA_DIR``.
"""
...
DEFAULT_DATA_ROOT = Path(__file__).resolve().parents[2] / "data" / "Lab"
DATA_ROOT = Path(os.environ.get("LANGUAGE_LAB_DATA_DIR", DEFAULT_DATA_ROOT))
...
SKILL_ALIASES = {
    "speaking": "speaking", "speak": "speaking", "spoken": "speaking",
    "oral": "speaking", "口语": "speaking",
    "writing": "writing", "write": "writing", "written": "writing",
    "composition": "writing", "写作": "writing",
}

bp = Blueprint("language_lab", __name__)
...
def topic_dir(language: Any, skill: Any) -> Path:
    return TOPICS_DIR / normalise_language(language, "es") / normalise_skill(skill)
...
def topic_audio_dir(language: Any, skill: Any, topic_id: str) -> Path:
    return AUDIO_DIR / "topics" / normalise_language(language, "es") / normalise_skill(skill) / topic_id
...
def normalise_examples(raw_topic: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for level in SUPPORTED_LEVELS:
        ...
        if not body:
            continue
        title = str(
            raw_topic.get(f"{key}Title") or raw_topic.get(f"{key}_title") or f"{level} model"
        ).strip()
        examples.append({
            "level": level, "title": title, "body": body,
            "notes": "", "wordCount": word_count(body),
        })
    ...
    return sorted(
        unique_examples,
        key=lambda item: SUPPORTED_LEVELS.index(item["level"]) if item["level"] in SUPPORTED_LEVELS else 99,
    )
...
def normalise_topic(
    ...
    fallback_skill: str = "writing",
) -> dict[str, Any]:
    if not isinstance(raw_topic, dict):
        raise ValueError("Each imported topic must be a JSON object.")
    language = normalise_language(
        raw_topic.get("language") or raw_topic.get("targetLanguage"), fallback_language
    )
    skill = normalise_skill(
        raw_topic.get("skill") or raw_topic.get("mode") or raw_topic.get("practiceType"),
        fallback_skill,
    )
    ...
    level_range = (
        raw_topic.get("levelRange") or raw_topic.get("level_range")
        or raw_topic.get("level") or " / ".join(levels)
    )
    ...
    return {
        ...
        "coreTakeaway": str(
            raw_topic.get("coreTakeaway") or raw_topic.get("core_takeaway") or ""
        ).strip(),
...
def public_topic(raw_topic: dict[str, Any]) -> dict[str, Any]:
    return normalise_topic(
        raw_topic, raw_topic.get("language", "es"), raw_topic.get("skill", "writing")
    )
...
def unique_topic_id_for_import(
    language: str, skill: str, desired_id: str,
    reserved_ids: set[tuple[str, str, str]],
) -> str:
    base = slugify(desired_id, "topic")
    candidate = base
    counter = 2
    while (
        topic_path(language, skill, candidate).exists()
        or (language, skill, candidate) in reserved_ids
    ):
...
@bp.get("/topics")
def get_topics():
    language = normalise_language(request.args.get("language"), "es")
    skill = normalise_skill(request.args.get("skill"))
    topics = load_topics(language, skill)
    return jsonify({
        "language": language, "skill": skill, "topics": topics,
        "summaries": [topic_summary(t) for t in topics],
    })


@bp.get("/topics/<language>/<skill>/<topic_id>")
...
@bp.patch("/topics/<language>/<skill>/<topic_id>/core-takeaway")
...
@bp.delete("/topics/<language>/<skill>/<topic_id>")
def delete_topic(language: str, skill: str, topic_id: str):
    ...
    lang = normalise_language(language, "es")
    mode = normalise_skill(skill)
    path.unlink()
    shutil.rmtree(topic_audio_dir(lang, mode, topic_id), ignore_errors=True)

    return jsonify({
        "deleted": topic_id, "language": lang, "skill": mode,
        "modules": module_payloads(), "topics": load_topics(lang, mode),
    })


@bp.post("/topics/import")
def import_topics():
    ...
    fallback_language = normalise_language(
        request.args.get("language") or payload_language, "es"
    )
    ...
    try:
        ...
        return jsonify({
            "error": f"Could not parse JSON near line {exc.lineno}, column {exc.colno}: {exc.msg}."
        }), 400
    except (TypeError, ValueError) as exc:
        return jsonify({"error": str(exc)}), 400

    return jsonify({"imported": imported, "modules": module_payloads()})


@bp.get("/schema/topic")
def topic_schema():
    return jsonify({
        "id": "future-work", "language": "en", "skill": "writing",
        "category": "Work", "title": "The future of work",
        "prompt": "Write about how work may change in the next ten years.",
        "format": "Opinion essay · 180-220 words", "levelRange": "B1 - C1",
        "examples": [
...
def create_tts_audio():
    ...
    language = normalise_language(payload.get("language"), "es")
    ...
    if topic_id and TOPIC_ID_RE.match(topic_id):
        ...
        audio_prefix = f"/audio/lab/topics/{language}/{skill}/{topic_id}"
    else:
        output_dir = AUDIO_DIR / "tts" / language
        audio_prefix = f"/audio/lab/tts/{language}"
    ...
    if audio_file_is_usable(output_path):
        return jsonify({
            ...
            "voice": voice, "language": language, "cached": True,
        })
    ...
    return jsonify({
        "audio_path": audio_path,
        "audio_url": request.host_url.rstrip("/") + audio_path,
        "voice": voice, "language": language, "cached": False,
    })
