"""Eng Vocab blueprint — English vocabulary by CEFR level.

Mounted at ``/api/eng-vocab``. Data lives in ``backend/data/EngVocab/``.

Mirrors ``apps/esp_vocab/routes.py`` exactly — only the target language
(English) and the translation language (Chinese, ``zh``) differ, so we
can read English lemmas + examples with an English voice, and read
Chinese translations with a Chinese voice.
"""
...
DEFAULT_DATA_DIR = Path(__file__).resolve().parents[2] / "data" / "EngVocab"
DATA_DIR = Path(os.environ.get("ENG_VOCAB_DATA_DIR", DEFAULT_DATA_DIR))
...
bp = Blueprint("eng_vocab", __name__)


class EngVocabError(ValueError):
    def __init__(self, message: str, status_code: int = 400) -> None:
        super().__init__(message)
        self.status_code = status_code


@bp.errorhandler(EngVocabError)
def handle_error(error: EngVocabError):
...
def normalise_tts_language(value: Any) -> str:
    ...
    if language not in LANGUAGE_CONFIG:
        raise EngVocabError("Language must be 'en' or 'zh'.")
    return language


def tts_cache_key(text: str, language: str, voice: str) -> str:
    payload = f"eng-vocab-edge-tts-v1\n{language}\n{voice}\n{text}"
...
def levels():
    if request.method == "OPTIONS":
        return "", 204
    return jsonify(load_levels(DATA_DIR, EngVocabError))
...
def level_detail(level_id: str):
    if request.method == "OPTIONS":
        return "", 204
    return jsonify(load_level(DATA_DIR, level_id, EngVocabError))
...
def group_detail(level_id: str, group_id: str):
    if request.method == "OPTIONS":
        return "", 204
    group = load_group(DATA_DIR, level_id, group_id, EngVocabError)
    return jsonify({
...
def create_tts_audio():
    ...
    if not text:
        raise EngVocabError("Text is required.")
    if len(text) > AUDIO_TEXT_LIMIT:
        raise EngVocabError(f"Text is too long. Keep it under {AUDIO_TEXT_LIMIT} characters.")
    ...
    audio_path = f"/audio/eng-vocab/{output_path.relative_to(AUDIO_DIR).as_posix()}"
    ...
    try:
        ...
        raise EngVocabError(f"Could not generate audio with {voice}: {exc}", 502) from exc
