...
const DEFAULT_LENGTH_TARGETS = {
  writing: {
    A1: "60-90 words", A2: "100-140 words", B1: "150-200 words",
    B2: "200-250 words", C1: "250-320 words", C2: "320-420 words"
  },
  speaking: {
    A1: "45-60 seconds", A2: "1-2 minutes", B1: "2-3 minutes",
    B2: "3-4 minutes", C1: "4-5 minutes", C2: "5-6 minutes"
  }
};
...
- Varied task types and themes.
- High-quality ${language.name}: authentic phrasing, realistic tasks.
...
Each topic object must use this exact structure:
{
  ...
  "skill": "${skill}",
  "category": "Short reusable category",
  "title": "Clear topic title in ${language.name}",
  "prompt": "Learner-facing ${skill} task in ${language.name}.",
  "format": "Concrete task format, e.g. ${formatExample}",
...
${lengthLines}`;
  ...
  return (
    <div className={`lab-shell ${rightOpen ? "right-open" : "right-closed"} ${leftCollapsed ? "left-closed" : "left-open"}`}>
      <TopNav
        modules={modules}
...
function PracticeView({
  module, topic, activeIndex, topicCount, draft, visibleExamples,
  busyAudio, audioError, onDraft, onClearDraft, onListen, onDeleteTopic, onPrev, onNext
}) {
  return (
...
function RightTools({
  open, module, topic, promptSettings, generatedPrompt,
  importText, importStatus, onToggle, onPromptSettings,
  onImportText, onImport, onCopyPrompt, onTopicUpdated
}) {
  ...
  function updateLengthTarget(level, value) {
    onPromptSettings((current) => ({
      ...current,
      lengthTargets: { ...current.lengthTargets, [level]: value }
    }));
  }
    ...
    <aside className="right-tools">
      ...
      <section className="tool-section">
        ...
        <div className="generator-field">
          ...
          <p className="field-hint">Each selected level gets one example.</p>
        </div>
        <div className="generator-field">
      ...
      <section className="tool-section">
        ...
        <p className="import-guidance">
          Paste a JSON array, a single topic object, or an object with a topics/items array.
        </p>
        <textarea
