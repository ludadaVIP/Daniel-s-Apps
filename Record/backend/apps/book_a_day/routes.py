...
``shelfId``. Default shelves are organised in 4 *groups* that reflect a book's
position in the reading life-cycle:

* ``pre`` — pre-reading categories (想读 / 属灵 / 文化 / 投资).
* ``reading`` — actively reading (在读).
...
def _load_shelves() -> list[dict[str, str]]:
    """Load shelves with auto-migration.

    Older ``shelves.json`` files contain only {id, name}. We:
