...
export default function BeliefQA() {
  ...
  const briefDraftIncomplete = isBriefLayer && briefLength > 0 && briefLength < BRIEF_ANSWER_MIN_LENGTH;
  const briefDraftTooLong = isBriefLayer && briefLength > BRIEF_ANSWER_MAX_LENGTH;
  ...
  async function removeCurrentAnswer() {
    ...
    try {
      const deleted = await deleteAnswer(questionId, activeLayer);
      if (selectedIdRef.current === questionId) setAnswer({ status: 'ready', layers: normalizedLayers(deleted.layers), updatedAt: null, dirty: false });
    } catch (error) {
  ...
  return <main className={`bqa-app ${sidebarOpen ? '' : 'bqa-sidebar-collapsed'} ${rightbarOpen ? '' : 'bqa-rightbar-collapsed'}`}>
    ...
    <section className="bqa-workspace">
      {selected && <div className="bqa-content">
        ...
        <section className={`bqa-answer-panel is-${layer.tone}`}>
          ...
          {answer.status === 'loading' || answer.status === 'deleting' ? <div className="bqa-answer-loading"><span /><p>{answer.status === 'deleting' ? '正在删除这篇回答…' : '正在读取这道题的准备稿…'}</p></div>
    ...
    <aside className={`bqa-rightbar ${rightbarOpen ? '' : 'is-collapsed'}`} aria-label="回答与题目工具">
      ...
      <section className="bqa-right-section bqa-writing-tools">
        ...
        {isBriefLayer && <div className={`bqa-word-count ${briefDraftIncomplete || briefDraftTooLong ? 'is-outside-range' : ''}`}><span>字数</span><strong>{briefLength}</strong><small>/ {BRIEF_ANSWER_MIN_LENGTH}–{BRIEF_ANSWER_MAX_LENGTH}</small></div>}
        <span className={`bqa-save-state ${isDirty ? 'is-dirty' : ''}`}>{briefDraftTooLong ? `超出 ${BRIEF_ANSWER_MAX_LENGTH} 字，暂不保存` : briefDraftIncomplete ? `还差 ${BRIEF_ANSWER_MIN_LENGTH - briefLength} 字，达到下限后自动保存` : answer.status === 'saving' ? '正在保存…' : isDirty ? '正在整理草稿…' : answer.updatedAt ? '已自动保存' : '开始输入后自动保存'}</span>
